...
class Validation(object):
    ...
    def validate(self):
                ...
                if conf_key:
                    for value in config_obj.configs.get(key):
                        ...
                        if key_map[key] == 'ip':
                            ...
                            pass
                        if valid == False:
                            FAIL = "[{red}FAIL{endred}] in configuration :: Check {key} setting: {value}".format(key=key, value=value, red=bcolors.FAIL, endred=bcolors.ENDC)
                            captured_errors.append(FAIL)
                            if self.verbose:
                                print(FAIL)
                        else:
                            if self.verbose:
                                print("[{green}OK{endgreen}] {value} is a valid setting for {key}".format(value=value, key=key, green=bcolors.OKGREEN, endgreen=bcolors.ENDC))
            except Exception as e:
                print e
        if missing_keys:
        ...
        if captured_errors:
            CONFIG_ERROR = "{error_count} errors detect. Please verify your settings in {config}".format(error_count=len(captured_errors), config=self.config)
            if self.verbose:
                print("\n\n"+"="*10+'ERROR REPORT'+"="*10)
