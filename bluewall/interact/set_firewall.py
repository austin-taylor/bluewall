...
class SetFirewall(CLIParser):
	...
	def disallow_dhcp(self):
		...
		self.command_list.extend(rules)
		if self.verbose > 0:
			print("{gp}{red} Disallowing {endc} DHCP...".format(gp=self.GREEN_PLUS, red=bcolors.FAIL, endc=bcolors.ENDC))
		return
	...
	def all_icmp_network(self, status=1, networks='0.0.0.0/0'):
		...
		try:
			for network in networks:
				...
				if status == 0:
					rules = self.rule_builder('I', chain_options='o', append_rule=self.ALLOW_NET_ICMP_8.format(net=network))
					rules += self.rule_builder('I', chain_options='o', append_rule=self.ALLOW_NET_ICMP_0.format(net=network))
					self.command_list.extend(rules)
				else:
					rules = self.rule_builder('I', chain_options='o', append_rule=self.ALLOW_NET_ICMP.format(net=network))
					self.command_list.extend(rules)
				if self.verbose > 0:
					print("{gp} Allowing {outbound} ICMP/traceroute to {net}...".format(net=network, outbound=self.OUTBOUND_C, gp=self.GREEN_PLUS))
		except:
	...
	def set_nostrike(self, networks=[]):
		...
		if networks:
			for network in networks:
				...
				self.command_list.extend(rules)
				if self.verbose:
					print("{gp} {red}DISALLOWING{endr} traffic to {net} ".format(net=network, red=bcolors.FAIL, endr=bcolors.ENDC, gp=self.GREEN_PLUS))
		return
	...
	def allow_outbound_transport(self, protocol='tcp', ports=[]):
		...
		if len(ports) == 0:
			rules = self.rule_builder('I', chain_options='o', append_rule='1 -p {protocol} -j ACCEPT'.format(protocol=protocol))
			self.command_list.extend(rules)
			if self.verbose > 0:
				print("{gp} Not limiting {outbound} {protocol} connections.".format(protocol=protocol.upper(), gp=self.GREEN_PLUS, outbound=self.OUTBOUND_C))
		else:
			ports = ','.join([str(p) for p in ports])
			rules = self.rule_builder('I', chain_options='o', append_rule='1 -p {protocol} -m multiport --dports {ports} -j ACCEPT'.format(ports=ports, protocol=protocol))
			self.command_list.extend(rules)
			if self.verbose > 0:
				print("[+] Limiting {outbound} connections to {protocol} ports: {ports}".format(ports=ports, outbound=self.OUTBOUND_C, protocol=protocol.upper()))
		return
	...
	def allow_network_transport(self, direction=None, protocol='tcp', ports=[], networks='0.0.0.0', policy='ACCEPT'):
		...
		try:
			if networks:
				for network in networks:
					if len(ports) == 0:
							rules = self.rule_builder('I', chain_options=direction_map[direction], append_rule='1 -d {net} -p {protocol} -j {policy}'.format(protocol=protocol, net=network, policy=policy))
							self.command_list.extend(rules)
							if self.verbose > 0:
								print("{gp} Limiting {direction} {protocol} connections to {net}.".format(protocol=protocol.upper(), gp=self.GREEN_PLUS, direction=bcolors.BOLD+direction+bcolors.ENDC, net=network))
					else:
						rules = self.rule_builder('I', chain_options=direction_map[direction], append_rule='1 -d {net} -p {protocol} -m multiport --dports {ports} -j {policy}'.format(ports=ports, policy=policy, protocol=protocol, net=network))
						self.command_list.extend(rules)
						if self.verbose > 0:
							if policy=='DROP':
								print(
								"{red} Disallowing{end} {direction} {protocol} connections to {net}  ports: {ports}".format(
									...
									direction=bcolors.BOLD + direction + bcolors.ENDC, red=bcolors.FAIL, end=bcolors.ENDC, net=network))
							print("{gp} Limiting {direction} {protocol} connections to {net}  ports: {ports}".format(ports=ports, protocol=protocol.upper(), direction=bcolors.BOLD+direction+bcolors.ENDC, gp=self.GREEN_PLUS, net=network))
		except Exception as e:
